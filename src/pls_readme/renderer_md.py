...
def render_tree(repo_path: Path, depth: int = 3, include_hidden: bool = False, max_entries: int = 200) -> str:
    """
    Lightweight tree rendering (not perfect, but good enough).
    """
    lines: List[str] = []
    ...
    def walk(dir_path: Path, prefix: str, level: int, entries_left: List[int]):
        ...
        # simple ignores: skip big/irrelevant
...
def render_markdown(scan: RepoScan, sug: Suggestions, depth: int, include_hidden: bool, show_tree: bool = True) -> str:
...
## Overview
> Replace this with 2–4 sentences: what this project does, who it is for, and why it exists.
